# 📚 Documentación

## 🚀 Instalación y Configuración

| Documento | Descripción |
|-----------|-------------|
| [SETUP.md](setup/SETUP.md) | Configuración inicial del proyecto |
| [QUICKSTART_WSL.md](setup/QUICKSTART_WSL.md) | Guía rápida para WSL |
| [DEPLOY.md](setup/DEPLOY.md) | Instrucciones de deploy en producción |

## ⚙️ Funcionalidades

| Documento | Descripción |
|-----------|-------------|
| [REMINDERS.md](features/REMINDERS.md) | Sistema de recordatorios automáticos |
| [SERVICIOS.md](features/SERVICIOS.md) | Servicios disponibles y duración por tipo |
| [WHATSAPP_TEMPLATES.md](features/WHATSAPP_TEMPLATES.md) | Configuración de templates de WhatsApp |
| [SYNC_RESERVAS.md](features/SYNC_RESERVAS.md) | Sincronización con Google Calendar |

## 💰 Pricing

| Documento | Descripción |
|-----------|-------------|
| [WHATSAPP_PRICING.md](pricing/WHATSAPP_PRICING.md) | Costos de WhatsApp Business API |
| [PROPUESTA_COSTOS_WHATSAPP.md](pricing/PROPUESTA_COSTOS_WHATSAPP.md) | Propuesta de costos para cliente |

## 🔧 Troubleshooting

| Documento | Descripción |
|-----------|-------------|
| [TROUBLESHOOTING.md](TROUBLESHOOTING.md) | Solución de problemas comunes |

---

## Estructura

```
docs/
├── README.md              ← Estás aquí
├── TROUBLESHOOTING.md
├── setup/
│   ├── SETUP.md
│   ├── QUICKSTART_WSL.md
│   └── DEPLOY.md
├── features/
│   ├── REMINDERS.md
│   ├── SERVICIOS.md
│   ├── WHATSAPP_TEMPLATES.md
│   └── SYNC_RESERVAS.md
└── pricing/
    ├── WHATSAPP_PRICING.md
    └── PROPUESTA_COSTOS_WHATSAPP.md
```
